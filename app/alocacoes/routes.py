...
from app.models import Alocacao, Projeto, Usuario, TipoDespesa
from app.alocacoes.forms import AlocacaoForm, TipoDespesaForm
...
def nova_alocacao(projeto_id):
    ...
    if form.validate_on_submit():
        ...
        alocacao = Alocacao(
            ...
            tipo_despesa_id=form.tipo_despesa_id.data,
...
def editar_alocacao(alocacao_id):
    ...
    if form.validate_on_submit():
        ...
        alocacao.tipo_despesa_id = form.tipo_despesa_id.data
...
@alocacoes_bp.route("/tipos-despesa", methods=["GET", "POST"])
@login_required
def tipos_despesa():
    if not _somente_administrador():
        return redirect(url_for("projetos.listar_projetos"))
    ...
    if form.validate_on_submit():
        ...
        if TipoDespesa.query.filter_by(nome=nome).first():
            flash("Esse tipo de despesa já existe.", "erro")
        else:
            db.session.add(TipoDespesa(nome=nome, ativo=True))
            db.session.commit()
            flash("Tipo de despesa adicionado.", "sucesso")
        return redirect(url_for("alocacoes.tipos_despesa"))

    tipos = TipoDespesa.query.order_by(TipoDespesa.nome).all()
    return render_template("alocacoes/tipos_despesa.html", form=form, tipos=tipos)


@alocacoes_bp.route("/tipos-despesa/<int:tipo_id>/alternar-status", methods=["POST"])
@login_required
def alternar_status_tipo_despesa(tipo_id):
    if not _somente_administrador():
        return redirect(url_for("alocacoes.tipos_despesa"))

    tipo = db.session.get(TipoDespesa, tipo_id)
    if tipo is None:
        flash("Tipo de despesa não encontrado.", "erro")
    else:
        tipo.ativo = not tipo.ativo
        db.session.commit()
        flash("Tipo de despesa atualizado.", "sucesso")
    return redirect(url_for("alocacoes.tipos_despesa"))